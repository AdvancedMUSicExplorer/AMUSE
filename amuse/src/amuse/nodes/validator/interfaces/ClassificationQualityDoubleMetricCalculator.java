...
/**
 * Methods which calculate double metrics based on classification results and ground truth information should extend this class.
 *  
 * @author Igor Vatolkin
 * @version $Id$
 */
public abstract class ClassificationQualityDoubleMetricCalculator implements ClassificationQualityMetricCalculatorInterface {
	 ...
	 */
	public ValidationMetricDouble[] calculateOneClassMetric(ArrayList<Double> groundTruthRelationships, ArrayList<ClassifiedSongPartitions> predictedRelationships) throws NodeException {
		...
		if(this.getSongLevel()) {
			metricOnSongLev = (ValidationMetricDouble[])calculateOneClassMetricOnSongLevel(groundTruthRelationships, predictedRelationships);
		} 
		if(this.getPartitionLevel()) {
			metricOnPartLev = (ValidationMetricDouble[])calculateOneClassMetricOnPartitionLevel(groundTruthRelationships, predictedRelationships);
		}
	 ...
	 */
	public ValidationMetricDouble[] calculateMultiClassMetric(ArrayList<ClassifiedSongPartitions> groundTruthRelationships, 
		...
		if(this.getSongLevel()) {
			metricOnSongLev = (ValidationMetricDouble[])calculateMultiClassMetricOnSongLevel(groundTruthRelationships, predictedRelationships);
		} 
		if(this.getPartitionLevel()) {
			metricOnPartLev = (ValidationMetricDouble[])calculateMultiClassMetricOnPartitionLevel(groundTruthRelationships, predictedRelationships);
		}
