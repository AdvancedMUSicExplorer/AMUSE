 ...
 * @version $Id: $
 */
public class TruePositives extends ClassificationQualityDoubleMetricCalculator {
	...
	public ValidationMetricDouble[] calculateOneClassMetricOnSongLevel(ArrayList<Double> groundTruthRelationships, ArrayList<ClassifiedSongPartitions> predictedRelationships) throws NodeException {
		...
		// Prepare the result
		ValidationMetricDouble[] truePositivesMetric = new ValidationMetricDouble[1];
		truePositivesMetric[0] = new ValidationMetricDouble();
		truePositivesMetric[0].setId(100);
		truePositivesMetric[0].setName("Number of true positives on song level");
		truePositivesMetric[0].setValue(new Double(numberOfTruePositives));
		return truePositivesMetric;
	}
	...
	public ValidationMetricDouble[] calculateOneClassMetricOnPartitionLevel(ArrayList<Double> groundTruthRelationships, ArrayList<ClassifiedSongPartitions> predictedRelationships) throws NodeException {
		...
		truePositivesMetric[0] = new ValidationMetricDouble();
		truePositivesMetric[0].setId(100);
		truePositivesMetric[0].setName("Number of true positives on partition level");
		truePositivesMetric[0].setValue(new Double(numberOfTruePositives));
		return truePositivesMetric;
	}
