...
import amuse.nodes.annotation.action.AnnotationAddAttributeAction;
...
import amuse.nodes.annotation.action.AnnotationRemoveAttributeAction;
import amuse.nodes.annotation.action.AnnotationRemoveAttributeEntryAction;
...
import amuse.nodes.annotation.attribute.AnnotationEventAttribute;
import amuse.nodes.annotation.attribute.AnnotationNominalAttribute;
import amuse.nodes.annotation.attribute.AnnotationNumericAttribute;
import amuse.nodes.annotation.attribute.AnnotationStringAttribute;
...
public class AnnotationSelectionPanel extends JSplitPane {
	...
	private void initAttributeListPanel() {
		...
		attributeList.addMouseListener(new MouseListener(){
			...
			public void mouseExited(MouseEvent e) {
				attributeEntryList.requestFocus();
			}
	...
	private class StringAttributeEntryPanel extends TimeAttributeEntryPanel<String>{
		...
		public void saveChanges() {
			...
			}//TODO one method instead of three
		}
				...
				if (result == JOptionPane.OK_OPTION) {
					if(amuseFeatureTable.getSelectedRowCount() > 0){
						...
						if(songPath.startsWith(AmusePreferences.get(KeysStringValue.MUSIC_DATABASE))) {
							relativeName = songPath.substring(AmusePreferences.get(KeysStringValue.MUSIC_DATABASE).length()+1);
						} else {
