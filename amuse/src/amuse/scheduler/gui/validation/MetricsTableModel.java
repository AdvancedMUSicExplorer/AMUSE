...
public class MetricsTableModel extends DefaultTableModel {
	...
	private String amuseHome = AmusePreferences.get(KeysStringValue.AMUSE_PATH);
    private File metricTableFile = new File(amuseHome + File.separator + "config" + File.separator + "metricTable.arff");
    private MetricTable metricTable;

    public MetricsTableModel() throws IOException {
        super();
        metricTable = new MetricTable(metricTableFile);
        Object[][] data = new Object[metricTable.size()][5];
        String[] columnHeaders = {"ID", "Name", "Category", "Song Level", "Partition Level"};
        int index = 0;
        for (Object[] column:data) {
            amuse.data.Metric m = metricTable.get(index);
            column[0] = m.getID();
            column[1] = m.getName();
    ...
    public void loadSelection(MetricTable table) {
    ...
    public MetricTable getMetricTable() {
        for (int i = 0; i < getRowCount(); i++) {
            ...
            Metric metric = metricTable.get(i);
            metric.setSongLevelSelected(songLevel);
            metric.setPartitionLevelSelected(partitionLevel);
        }
        return metricTable;
    }
}
