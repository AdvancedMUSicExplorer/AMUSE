...
%    option.gran = gran;
    ...
    option.normal = normal;
        ...
        filterborder.key = 'FilterBorder';
        filterborder.type = 'Boolean';
        filterborder.default = 1;
        filterborder.when = 'After';
    option.filterborder = filterborder;
    
        direct.type = 'String';
        direct.default = 'Both';
        direct.when = 'After';
        direct.choice = {'Both','Forth','Back'};
        ...
        for k = 1:length(s)
            for i = 1:length(Ks)
                ...
                if strcmpi(postoption.direct,'Back')
                    cg(:,cgs/2+1:end) = 0;
                elseif strcmpi(postoption.direct,'Forth')
                    cg(:,1:cgs/2) = 0;
                end
