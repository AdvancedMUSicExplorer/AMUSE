...
% Last modified on 11.03.2012 by Igor Vatolkin (update to Chroma Toolbox 2.0 and integration of CRP features)
% $Id: $
function ChromaToolboxBase(input_file,output_dir)

% Add the path to Chroma Toolbox
addpath('/home/vatol/workspace/amuse/tools/ChromaToolbox');
...
output_file_prefix = [output_dir,'/',input_file(p(end)+1:k(end)-1),'_'];
	  ...
	  curr_index = 1;
	  for i=1:length(f_CRP) % Go through time windows
