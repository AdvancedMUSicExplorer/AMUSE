...
/**
 * This class runs specific routines during Chroma Toolbox installation procedure
 ...
 * @version $Id: $
 */
public class ChromaToolboxPluginManager implements PluginInstallerInterface {
	...
	public void runInstallationRoutines(Properties properties) throws PluginException {
		...
		File baseScript = new File(System.getenv("AMUSEHOME") + "/tools/ChromaToolbox/ChromaToolboxBase.xml");
		File updatedBaseScript = new File(System.getenv("AMUSEHOME") + "/tools/ChromaToolbox/ChromaToolboxBaseUpdated.xml");
		...
		try {
			...
			while ((line = baseScriptReader.readLine()) != null) {
				if(line.startsWith(new String("addpath('%AMUSEHOME%"))) {
					AmuseLogger.write(this.getClass().getName(), Level.INFO, "Setting the AMUSE path for Chroma Toolbox folders in base extractor script..");
