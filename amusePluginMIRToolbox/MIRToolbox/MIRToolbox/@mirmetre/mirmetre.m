...
x = mirautocor(x,'Min',60/option.ma,'Max',60/option.mi * 2,...
                 'NormalWindow',0,'Gauss',option.gauss);
...
bpms{i2} = bpm2;
                        ...
                        for i3 = 1:length(mk{i2})
                            if mk{i2}(i3).timidx(end) == l
                                continue
elseif mk{i2}(i3).timidx(end) < l-20
    dist2(i3) = NaN;
    continue
                            end
                        ...
                        if ~foundk(i2)
                            if abs(log2(ptli / bpm2(indx(i2)))) > .4 %.2 <<<<<<<<< %.1 <<<<<<<<<<<<<<<<<<<<<<<
...
mk{i2}(indx(i2)).score(end) > .01 %%%%%%.15 %.3 %.15  <<<<<<<<<<<<<
                                        i3 = find([mk{i2}.lvl] == ...
                                                  mk{i2}(indx(i2)).ref,1);
...
if bpm3 < 60
    ...
    continue                            
                            ...
                            % NEW:
                            if rdiv == 1
if mk{i2}(ord(i3)).timidx(end) <= l-20
    mk{i2}(ord(i3)).timidx(end+1) = l;
    mk{i2}(ord(i3)).bpms(end+1) = ptl(i);
    mk{i2}(ord(i3)).lastbpm = ptli;
...
%                                         end
                    ...
                    if i == 1 && ...
                            ...
                            isempty(find(found))) && ...
                            ptl(i) < 300 && ...
                            d{j}{k}(ppp{j}{k}{1,l,h}(i),l,h) > 0 %.1 %.15)  % <<<<<<<<<<<<<<<<<<<<<<<<<<
                    ...
                    if l == 1 || isnan(globpm(i,l-1)) || ~globpm(i,l-1)
                        ...
                        for i2 = 1:length(mk{i})
                            if mk{i}(i2).timidx(end) == l && ...
                                ...
                                glodif = glodif + sco2 * sign(dif); % + dif * sco2;
                                sco = sco + sco2;
